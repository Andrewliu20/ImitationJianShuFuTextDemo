...
@property(retain,nonatomic)WPLegacyEditorViewController *legacyEditor;
...
- (void)dealloc
{
    ...
    [[NSNotificationCenter defaultCenter]removeObserver:self name:@"showEditorTitleNoti" object:nil];
    [[NSNotificationCenter defaultCenter]removeObserver:self name:@"hideEditorTitleNoti" object:nil];
...
//     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillShowOrHide:) name:UIKeyboardWillHideNotification object:nil];
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeState:) name:@"showEditorTitleNoti" object:nil];
    
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(hideEditor:) name:@"hideEditorTitleNoti" object:nil];
    
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(getContentLenght:) name:@"getContentLengthNoti" object:nil];
	if (self) {
		CGRect childFrame = frame;
		childFrame.origin = CGPointZero;
		self.legacyEditor = [[WPLegacyEditorViewController alloc] init];
...
- (void)getContentLenght:(NSNotification *)noti{
    
    
    NSString *string = [noti.userInfo objectForKey:@"contentLength"];
...
- (void)createSourceTitleViewWithFrame:(CGRect)frame
{
    ...
    _sourceViewTitleField.accessibilityLabel = NSLocalizedString(@"Title", nil);
    
//    _sourceViewTitleField.keyboardType = UIKeyboardAppearanceDefault;
    _sourceViewTitleField.returnKeyType = UIReturnKeyDone;
...
- (void)setupHTMLEditor
{
    ...
    if (self.isTitleDiv) {
        ...
        editorURL = [bundle URLForResource:@"editor_1" withExtension:@"html"];
    }
...
- (void)hideEditor:(NSNotification *)noti{
...
//    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(keyboardWillHide:) name:@"keyBorderHidder" object:nil];
}
...
//    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"keyBorderHidder" object:nil];
}
...
- (void)keyboardDidShow:(NSNotification *)notification
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"keyBoreNoti" object:nil];
...
- (void)handleSelectionStyleCallback:(NSURL*)url
{
    ...
    NSDictionary *dict = @{@"style":styles};
    
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ChangStylekeyBorderNotis" object:nil userInfo:dict ];
...
- (void)heading1
{
    ...
    if (contentIsEmpty) {
        ...
        trigger = @"ZSSEditor.setHeading('h1');";
        
    }else{
        
        trigger = @"ZSSEditor.setHeading('h1'); ";
//        document.getElementById('zss_field_content').style.lineHeight = '40px';
    }
...
- (void)moveHeading1{
    ...
    [self.webView stringByEvaluatingJavaScriptFromString:@"$('h1').st"];
}
...
- (NSArray *)changStylekeyBorderNoti
{
