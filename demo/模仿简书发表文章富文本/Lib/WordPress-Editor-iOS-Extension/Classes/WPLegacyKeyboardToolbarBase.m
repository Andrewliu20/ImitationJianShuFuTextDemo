...
- (void)buildFormatButtons {
    ...
    if(self.hiddenKeyboard == nil){
    
        self.hiddenKeyboard = [WPLegacyKeyboardToolbarButtonItem button];
        [self.hiddenKeyboard setImageName:@"icon_format_keyboard@2x" withColor:nil highlightColor:highlightColor];
        self.moreButton.frame = CGRectMake(x, 0, width, height);
        self.moreButton.actionTag = @"keyboardHidden";
        self.moreButton.accessibilityIdentifier = @"keyboardHidden";
        self.moreButton.actionName = NSLocalizedString(@"keyboardHidden", @"Adding a keyboardHidden excerpt cut-off in the Post Editor. This string will be used in the Undo message if the last change was adding this formatting.");
