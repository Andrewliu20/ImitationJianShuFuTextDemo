...
@property(nonatomic,retain) WYPopoverController *wypopoverStyleController;

@property (nonatomic,strong)  WYPopoverController *tempPopoverController;

@property(retain,nonatomic)imageSelectController *imageselectController;
@property (nonatomic,strong)  MoreStyleVC *moreStyleVC;
@property (nonatomic,retain)FontStyleSelectView *fontstyleView;
...
@implementation WPEditorViewController
{
    CGRect parentFrame;
    UITextField *linkURL;
    UITextField *linkNameTextField;
    WYPopoverController *tempPopoverController;
    BOOL h1Bool;
    BOOL h2Bool;
    BOOL h3Bool;
    BOOL h4Bool;
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getTitleOrbodyVale:) name:@"setTitleORBodyValueNoti" object:nil];
    
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changFontStyle:) name:@"ChangeFontStyleNoti" object:nil];
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(moreStyleSelect:) name:@"MoreStyleSetingNoti" object:nil];
    [self createToolbarView];
    [self buildTextViews];
}
- (void)getTitleOrbodyVale:(NSNotification *)noti{
    
    self.titleText = [noti.userInfo objectForKey:@"title"];
    
    self.bodyText = [noti.userInfo objectForKey:@"bodyContent"];
}
...
//    [[NSNotificationCenter defaultCenter]removeObserver:self name:@"ChangeFontStyleNoti" object:nil];
...
- (NSMutableArray *)itemsForToolbar
{
    ...
    NSLog(@"test===%d",[self.toolbarView hasSomeEnabledToolbarItems]);
   
//    if ([self.toolbarView hasSomeEnabledToolbarItems]) {
    
//        NSLog(@"test= %d",[self canHidekeyBorderBarButton]);
        if(true){
            [items addObject:[self addHiddenKeyboardBarButton]];
        }
...
//		}
        ...
        if ([self canShowStrikeThroughBarButton]) {
    			[items addObject:[self addFontStyleBarButton]];
        }
    
    if ([self canShowStrikeThroughBarButton]) {
        [items addObject:[self addMoreBarButton]];
    }
...
//- (BOOL) canHidekeyBorderBarButton{
//    
//    return [self canShowToolbarOption:ZSSRichTextEditorToolbarKeyBoardHidden];
//}
...
//在item上添加键盘button
- (UIBarButtonItem*)addHiddenKeyboardBarButton
{
    ZSSBarButtonItem *barButtonItem = [self barButtonItemWithTag:kWPEditorViewControllerElementHiddenKeybButton
                                                    htmlProperty:@"hiddenKeyborder"
                                                       imageName:@"icon_format_keyboard@2x"
                                                          target:self
...
//在item中添加字体的样式
- (UIBarButtonItem *)addFontStyleBarButton{

    ZSSBarButtonItem *barButtonItem = [self barButtonItemWithTag:kWPEditorViewControllerElementFontStyleButton
                                                    htmlProperty:@"fontStyle"
                                                       imageName:@"sey-a-a@2x"
                                                          target:self
                                                        selector:@selector(addFontStyle:)
                                              accessibilityLabel:nil];
...
// 添加集成照片,超链接,分割线的item
- (UIBarButtonItem *)addMoreBarButton{
    
    ZSSBarButtonItem *barButtonItem = [self barButtonItemWithTag:kWPEditorViewControllerElementFontStyleButton
                                                    htmlProperty:@"moreBarButton"
                                                       imageName:@"addMore@2x"
                                                          target:self
                                                        selector:@selector(addMoreStyle:)
                                              accessibilityLabel:nil];
...
- (UIBarButtonItem*)strikeThroughBarButton
{
	...
	ZSSBarButtonItem *barButtonItem = [self barButtonItemWithTag:kWPEditorViewControllerElementStrikeThroughBarButton
													htmlProperty:@"strikeThrough"
													   imageName:@"icon_format_strikethrough@2x"
														  target:self
...
- (void)buildTextViews
{
    if (!self.editorView) {
        ...
        self.editorView.isTitleDiv = YES;
...
{
    _bodyContent = bodyText;
    
    
//    [self.editorView.contentField setHtml:bodyText];
    [self.editorView.contentField setHtml_test:_bodyContent];
...
- (void)addMoreStyle:(WPEditorToolbarView *)button{
    
    if (self.editorView.isInVisualMode) {
        
        if (!_moreStyleVC) {
            UICollectionViewFlowLayout *flow = [[UICollectionViewFlowLayout alloc]init];
            _moreStyleVC = [[MoreStyleVC alloc] initWithCollectionViewLayout:flow];
            
        }
        
        parentFrame = [button convertRect:button.bounds toView:self.view];
       
        
        _wypopoverStyleController = [[WYPopoverController alloc] initWithContentViewController:_moreStyleVC];
        _wypopoverStyleController.delegate = self;
        _wypopoverStyleController.theme.fillBottomColor = [UIColor blackColor];
        _wypopoverStyleController.popoverContentSize = CGSizeMake(185, 45);
        
      _wypopoverStyleController.dismissOnTap = YES;
        
        
    
        
        [_wypopoverStyleController presentPopoverFromRect:CGRectMake(button.frame.size.width/2, 100, 0, 0) inView:button permittedArrowDirections:WYPopoverArrowDirectionDown animated:YES];
    }
...
//实现字体样式的view
- (void)addFontStyle:(WPEditorToolbarView *)button{
    ...
    if (self.editorView.isInVisualMode) {
        ...
        if (!_fontstyleView) {
            UICollectionViewFlowLayout *flow = [[UICollectionViewFlowLayout alloc]init];
            _fontstyleView = [[FontStyleSelectView alloc] init];
            
        }
        
        CGRect parentFrame = [button convertRect:button.bounds toView:self.view];

        self.wypopoverController = [[WYPopoverController alloc] initWithContentViewController:_fontstyleView];
        self.wypopoverController.delegate = self;
        self.wypopoverController.popoverContentSize = CGSizeMake(300, 45);
        self.wypopoverController.theme.fillBottomColor = [UIColor blackColor];
//        _wypopoverController.dismissOnTap = YES;
        
        [self.wypopoverController presentPopoverFromRect:CGRectMake(button.frame.size.width/2, 50, 0, 30) inView:button permittedArrowDirections:WYPopoverArrowDirectionDown animated:YES];
    }
...
{
    if (_wypopoverController == nil) {
        _wypopoverController = [[WYPopoverController alloc] init];
    }
    
    return _wypopoverController;
}

- (WYPopoverController *)wypopoverStyleController
{
    if (_wypopoverStyleController == nil) {
        _wypopoverStyleController = [[WYPopoverController alloc]init];
    }
    return _wypopoverStyleController;
}

//实现添加照片,链接,分割线
- (void)moreStyleSelect:(NSNotification *)noti{

    NSString *item = [noti.userInfo objectForKey:@"item"];
    ...
    if ([item isEqualToString:@"1"]) {
        // 气泡
        //添加超连接
        [self linkBarButtonTapped:nil];
    
        
    }
    if ([item isEqualToString:@"2"]) {
...
//改变字体样式的实现
- (void)changFontStyle:(NSNotification *)noti{
...
//        }
    ...
    if ([item isEqualToString:@"6"]) {
        
        h3Bool = !h3Bool;
        [self heading3];
    }
    if ([item isEqualToString:@"7"]) {
        h4Bool = h4Bool;
        [self heading4];
    }
...
//            _imageselectController.view.backgroundColor = [UIColor redColor];
...
- (void)sethiddenKeyBorder{
    
    [self.editorView setKeyboardHidden];
}
- (void)setBold
{
...
- (void)undo:(ZSSBarButtonItem *)barButtonItem
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"undoBackNoti" object:nil];
							   ...
							   title:(NSString*)title
{
	...
	NSString *insertButtonTitle = isInsertingNewLink ? NSLocalizedString(@"确定", nil) : NSLocalizedString(@"更新", nil);
	NSString *removeButtonTitle = isInsertingNewLink ? nil : NSLocalizedString(@"移除超链接", nil);
    if (!self.alertView) {
        self.alertView = [[UIAlertView alloc] initWithTitle:@"插入超链接"
                                                    message:nil
                                                   delegate:self
	...
	linkURL = [self.alertView textFieldAtIndex:0];
    ...
    linkNameTextField = [self.alertView textFieldAtIndex:1];
      ...
      fieldCreated:(WPEditorField*)field
{
    if (field == self.editorView.titleField) {
        ...
        if (_bodyContent) {
            ...
            [field addBodyContent:_bodyContent];
        }else{
            [field setPlaceholderText:self.bodyPlaceholderText];
