...
//判断是否样式有效
var boldBool = false;
var italicBool = false;
var deleteLineBool = false;
var styleBgBool = false;
var h1Bool = false;
var h2Bool = false;
var h3Bool = false;
var h4Bool = false;
var phoneBool = false;
var superLinkBool = false;
var qutoBool = false;
var headingandQuoteBool = false;
...
////没有样式时的内容
//ZSSEditor.setNotStyleHtml = function(){
//    
//    if(i == 0){
//        i = i + 1;
//        html_0 = document.getElementById("zss_field_content").innerHTML;
//        contentArray.push(html_0);
//    }
//}

ZSSEditor.setBold = function() {
    document.execCommand('bold', false, null);
    //    ZSSEditor.setNotStyleHtml();
    //
    //
    //    boldBool = !boldBool;
    //    if(boldBool){
    //        var style_code = zss_field_content.innerHTML;
    //        contentArray.push(style_code);
    //        i = i + 1;
    //    }
...
ZSSEditor.setHorizontalRule = function() {
    ...
    document.execCommand('insertHorizontalRule', false, 'separator');
...
ZSSEditor.setHeading = function(heading) {
    ...
    if(formatTag == "h1"){
        ...
        h1Bool = !h1Bool;
    }else if(formatTag == "h2"){
        h2Bool = !h2Bool;
    }else if(formatTag == "h3"){
        h3Bool = !h3Bool;
    }else if(formatTag == "h4"){
        h4Bool = !h4Bool;
    }
    ...
    //        contentArray.push(zss_field_content.innerHTML);
    //
    //        i = i + 1;
    //
    //	ZSSEditor.sendEnabledStyles();
...
ZSSEditor.keyAction = function() {
    
    var textbox = document.getElementById('zss_field_content');
...
ZSSEditor.keyActiontitle = function() {
    
    var textbox = document.getElementById('zss_field_title');
    var sel = window.getSelection();
    var range = document.createRange();
    range.selectNodeContents(textbox);
    range.collapse(false);
...
ZSSEditor.insertBodyContent = function(str){
    
//  zss_field_title.removeChild(placeholderText);
    
//    alert(str);
    document.getElementById("zss_field_content").innerHTML = srt;
    
    
//    ZSSField.keyAction();
}

ZSSEditor.insertTitle = function(str){
...
//    document.execCommand('formatBlock', false, '<p>');
     ...
     document.getElementById("zss_field_title").innerHTML = str;
    
    ZSSField.keyActiontitle();
    
}

ZSSEditor.undo = function() {
    //	document.execCommand('undo', true, param);
    ...
    html_0 = zss_field_content.innerHTML;
    //    alert(contentArray.in_array(html_0));
    if(contentArray[contentArray.length-1] != html_0){
        contentArray.push(html_0);
        i = i+1;
    }
    i = i - 1;
    ...
    if(i == 0 ){
        document.getElementById("zss_field_content").innerHTML = contentArray[0];
    ...
    if(i>=0){
        ...
        document.getElementById("zss_field_content").innerHTML = contentArray[i];
        ZSSEditor.keyAction();
    }
...
ZSSEditor.redo = function() {
    ...
    //    if(b==1&&i==0){
    //        document.getElementById("zss_field_content").innerHTML = html_0;
    //
    //       ZSSEditor.keyAction();
    //
    //        i = i  + 1;
...
//    ZSSEditor.keyAction();
    ...
    //    alert(zss_field_content.innerHTML);
};
